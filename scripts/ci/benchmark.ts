...
samples.sort((left: any, right: any) => left - right);
const percentile = (value: any) => samples[Math.min(samples.length - 1, Math.ceil(samples.length * value) - 1)];
const report = {
...
const maxP95Ms = Number(process.env.ODINN_BENCHMARK_P95_MAX_MS || 2000);
